...
ElenaOS 基于 LVGL（LittlevGL）图形库提供了丰富的 UI 组件和接口，允许开发者在 JavaScript 中创建和管理用户界面。本文档将详细介绍 ElenaOS 暴露给脚本运行时的 LVGL UI API，基于实际项目工程中的使用方式。
...
在 ElenaOS 中，LVGL API 通过 `lv` 命名空间暴露给 JavaScript 脚本：
