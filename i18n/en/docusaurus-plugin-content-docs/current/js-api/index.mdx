...
ElenaOS provides rich JavaScript APIs for developing applications and watch faces. This document will introduce the general usage methods of JS APIs, including basic concepts, naming conventions, error handling, and other content.

## Basic Concepts

### API Namespaces

ElenaOS's JavaScript APIs are exposed to scripts through the following namespaces:
...
ElenaOS's JavaScript APIs adopt an object-oriented style, using constructors to create objects and instance methods to manipulate objects:
...
ElenaOS's JavaScript APIs use callback functions to handle events:
...
ElenaOS's JavaScript APIs may throw the following types of errors:
...
In ElenaOS, the lifecycle of objects is managed by the garbage collector. When an object is no longer referenced, the garbage collector will automatically reclaim its memory.
...
ElenaOS's JavaScript APIs adopt an object-oriented style, using constructors to create objects and instance methods to manipulate objects. By understanding and using these APIs, developers can create feature-rich, high-performance applications.
