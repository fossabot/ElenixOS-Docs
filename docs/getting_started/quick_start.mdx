...
[ElenaOS-PC-Simulator](https://github.com/Sab1e-dev/ElenaOS-PC-Simulator) 基于 [lv_port_pc_vscode](https://github.com/lvgl/lv_port_pc_vscode) 对 ElenaOS 进行了移植，
您可以在任意 PC 上编译并运行模拟 ElenaOS，以便快速迭代和调试。
...
```bash
git clone --recursive https://github.com/Sab1e-dev/ElenaOS-PC-Simulator.git
```
...
:::warning
非必要请勿修改 MSYS2 的安装路径，若修改，需要同步修改`ElenaOS-PC-Simulator`根目录中`simulator.code-workspace`文件的路径配置。
:::
