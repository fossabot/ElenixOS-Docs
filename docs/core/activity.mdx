...
│   ├── eos_activity.c      # Activity 控制器核心实现
│   └── eos_activity.h      # Activity 接口定义
├── src/core/ui/app_header/
│   ├── eos_app_header.c    # 应用顶部导航栏
│   └── eos_app_header.h    # 导航栏接口定义
```
...
Activity 系统与[事件系统](/docs/core/events)紧密集成。当 Activity 页面切换完成时，会广播 `EOS_EVENT_ACTIVITY_SCREEN_SWITCHED` 事件：
