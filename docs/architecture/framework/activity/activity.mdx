...
Activity 系统采用**智能懒加载**设计，简化初始化流程：

```mermaid
flowchart TD
    A["eos_watchface_init()"] --> B["eos_activity_create_root()"]
    B --> C{"root_screen 存在?"}
    C -->|"否"| D["自动创建 root_screen"]
    C -->|"是"| E["复用已有 root_screen"]
    D --> F["立即创建 View"]
    E --> F
    F --> G["返回完整 Activity"]

    H["eos_activity_controller_init()"] --> I{"root_screen 存在?"}
    I -->|"否 (create_root 未调用)"| J["自动创建 root_screen + stack"]
    I -->|"是 (已被 create_root 创建)"| K["跳过，仅创建 stack"]
    J --> L["设置 Root Activity"]
    K --> L
```
